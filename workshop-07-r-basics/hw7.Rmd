...
5. Write a logical statement to check if the `samples` column in `dfFile1` matches the `samples` column in `dfFile2`. Your statement should return a single logical value, not a vector of logical values.  

## Step 2
What can you tell about their dimensions and what's in the dataset? It looks like someone accidentally split the dataset into two separate files since the `samples` column are the same. Let's combine them to make one giant data frame.

Create a new chunk. Create a new variable called `dfCompiled` that combines the two data frames from Step 1. Because the `samples` column is the same between the two original data frames, we don't want to keep a duplicate column. Your final `dfCompiled` should not contain duplicate `samples` columns.
...
1. Create a named vector where each name corresponds to the column name and the value corresponds to the mean of the column. Because `samples` column contains type character, this column should not be included in this vector.
